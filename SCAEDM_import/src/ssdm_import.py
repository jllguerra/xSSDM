...
#             return 1
    ...
    def main(self,argv):
        ...
        # Log file        
#         if not os.path.exists(Globals.TMP_DIR): 
...
#                 sys.exit(2)
        ...
        if self.DB.Conn.error == "":  
            ...
            if not err: 
                ...
                if result=="": self.logger.info ("Simulation %s imported successfuly" % (self.Sim.ID))
                else: self.logger.error (": %s" % (result))
            else: self.logger.error ("Error loading Data: %s" % self.DB.Conn.error)
        else: self.logger.error ("Login Error: %s" % self.DB.Conn.error)
        return
